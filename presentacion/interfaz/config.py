...
def config(root, GoMain, cerrar_todo, configurar_ventana):
    ...
    def aplicar_configuracion():
        ...
        etiqueta_estado.config(
            text="Configuración aplicada para esta sesión.",
            fg="#8ee08e"
        )
    ...
    tk.Checkbutton(
        contenido_visual,
        text="Mostrar cuadrícula del tablero",
        variable=cuadricula_var,
        bg=COLOR_TARJETA,
        fg=COLOR_TEXTO,
        font=("Arial", 11)
    ).pack(anchor="w")
    ...
    tk.Checkbutton(
        contenido_visual,
        text="Mostrar proyectiles al ejecutar turno",
        variable=proyectiles_var,
        bg=COLOR_TARJETA,
        fg=COLOR_TEXTO,
        font=("Arial", 11)
    ).pack(anchor="w")
    ...
    tk.Label(
        contenido_red,
        text="IP predeterminada del servidor",
        bg=COLOR_TARJETA,
        fg=COLOR_TEXTO,
        font=("Arial", 12, "bold"),
        anchor="w"
    ...
    boton_restaurar = tk.Button(
        ...
        width=14,
        bg="#ffd54f",
        command=restablecer_configuracion
    )
    boton_restaurar.place(x=575, y=500)

    etiqueta_estado = tk.Label(
        window3,
        text="Los cambios aplican durante la sesión actual.",
        font=("Arial", 11, "bold"),
        bg=COLOR_FONDO,
        fg=COLOR_SUAVE
    )
